""" One way to generate random numbers is the so-called middle square
technique, which consists of take a number (seed) of N digits, square
it, and use the middle N digits as the next.
...
Design a program that asks a user for the value of N to generate random
numbers with the technique of the middle square, the program must 
verify that such number has at least 4 digits, otherwise
throw an error message and re-request the N number. You must grant the
user a maximum of 3 Opportunities to enter the number correctly.
 """
